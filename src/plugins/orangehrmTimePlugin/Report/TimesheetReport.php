...
class TimesheetReport implements EndpointAwareReport
{
    ...
    public const FILTER_ACTIVITY_ID = 'activityId';
    ...
    {
        $filterParams = new TimesheetReportSearchFilterParams();
        ...
        // Only employee and date filters - other filters removed
        ...
        return new ParamRuleCollection(
            ...
            $endpoint->getValidationDecorator()->notRequiredParamRule(
                new ParamRule(
                    ...
                    new Rule(Rules::IN, [TimesheetReportSearchFilterParams::INCLUDE_TIMESHEETS])
                )
            ),
            ...$endpoint->getSortingAndPaginationParamsRules(
                TimesheetReportSearchFilterParams::ALLOWED_SORT_FIELDS
            )
        );
    }
        ...
        return new Header(
            [
                ...
                (new Column(self::PARAMETER_ACTIVITY_NAME))->setName($this->getI18NHelper()->transBySource('Activity Name'))
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_DURATION))->setName($this->getI18NHelper()->transBySource('Hours'))
                    ->setCellProperties(['class' => ['col-alt' => true]])
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
                (new Column(self::PARAMETER_COMMENT))->setName($this->getI18NHelper()->transBySource('Comment'))
                    ->setSize(self::DEFAULT_COLUMN_SIZE),
            ]
        );
    }
     ...
     * @return TimesheetReportData
     */
    public function getData(FilterParams $filterParams): ReportData
    {
        return new TimesheetReportData($filterParams);
    }
    ...
    public function checkReportAccessibility(EndpointProxy $endpoint): void
    {
        ...
        // Use time_employee_reports data group (same as EmployeeReport)
